...
const Header: React.FC<HeaderProps> = ({ role, email, setRole }) => {
  ...
  const handleProfile = () => {
    // Navigate to the unified profile page
    if (role === "admin") navigate("/profile");
    else if (role === "buyer") navigate("/profile");
    else if (role === "supplier") navigate("/profile");
    else if (role === "super-admin") navigate("/profile");
  };
